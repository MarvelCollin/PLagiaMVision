...
def extract_zip_contents(zip_path: Path) -> List[str]:
    """Extract all code files from a zip file"""
    contents = []
    try:
...
def read_file_content(file_path: Path) -> str:
    ...
    try:
        if file_path.suffix.lower() == '.zip':
            return "\n".join(extract_zip_contents(file_path))
...
def organize_submissions(file_paths: List[Path]) -> Dict[str, List[str]]:
    """Organize submissions by user (zip file)"""
    submissions = {}
    
    for path in file_paths:
        if path.suffix.lower() == '.zip':
            # Use zip filename as user identifier
            user_id = path.stem
